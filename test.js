...
const ordenarNombreDescendente = async () => {
	//LIMPIAR EL ARRAY ORDENADO
	arrayOrdenado = [];
	let arraySocios = await obtenerSocios();
	console.log(arraySocios);
...
pintarSociosAsync();
escucharBotonesAsync();

/******************Escuchar botones Borrar Async***********************/

async function escucharBotonesAsync() {
	//
	...
	arrayOrdenado.length === 0 ? lista : (lista = arrayOrdenado);
